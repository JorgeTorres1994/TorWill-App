...
class ListaNotasCuestionarioEstudiante extends StatefulWidget {
  //final String? idUsuario;
  final String? idCuestionario; // Cambio aquí

  ListaNotasCuestionarioEstudiante(
      {Key? key, this.idCuestionario})
      : super(key: key);
  ...
  Future<Uint8List> generatePdfTotal(List<DocumentSnapshot> resultados) async {
    ...
    for (var resultado in resultados) {
      ...
      var cuestionarioData =
          cuestionarioSnapshot.data() as Map<String, dynamic>?;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
              ...
              onPressed: () async {
                var snapshot = await _firestore
                    ...
                    .where('idCuestionario',
                        isEqualTo: widget.idCuestionario) // Cambio aquí
                    .get();
                var resultados = snapshot.docs;
                var pdfBytes = await generatePdfTotal(resultados);
                downloadFile(pdfBytes, 'notas_cuestionario.pdf');
      ...
      body: StreamBuilder<QuerySnapshot>(
        stream: _firestore
                .collection('ResultadoCuestionarioEstudiante')
                .where('idCuestionario', isEqualTo: widget.idCuestionario)  // Cambio aquí
                //.where('idUsuario', isEqualTo: widget.idUsuario)
                .snapshots(),
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
                          ...
                          trailing: IconButton(
                            ...
                            onPressed: () async {
                              var pdfBytes = await generatePdfPorEstudiante(
                                  ...
                                  photoUrl);
                              downloadFile(
                                  pdfBytes, 'notas_cuestionario_$index.pdf');
                            },
                          ),
                        ),
                      );
